<?php
//displays fields to enter data

require_once( realpath( dirname( __FILE__ ) ).'/../config.php' );
require_once( realpath( dirname( __FILE__ ) ).'/../functions.php' );
require_once( realpath( dirname( __FILE__ ) ).'/../html.php' );
session_start();
$output  = "";
if($_SESSION["write"]=="1"){
$output .= "<table>";
$output .= "	<tr>";
$output .= "		<td>Datum *</td>";
$output .= "		<td>".html_createInputText("nummer_datum")."&nbsp;<i>(Nur 1 Datum im Format DD.MM.YYYY!)</i></td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Straße</td>";
$output .= "		<td>".html_createInputText("nummer_strasse")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Nummer</td>";
$output .= "		<td>".html_createInputText("nummer_nummer", "","tel")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>PLZ</td>";
$output .= "		<td>".html_createInputText("nummer_plz", "","number")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Ort</td>";
$output .= "		<td>".html_createInputText("nummer_ort")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Adresszusatz</td>";
$output .= "		<td>".html_createInputText("nummer_zusatz")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Auftraggeber</td>";
$output .= "		<td>";
$output .= "			<table>";
$output .= "				<tr>";
$output .= "					<td>".html_createRadioButton("nummer_rb", "rb_select",true)."</td>";
$output .= "					<td class=\"boxed\" id=\"rb_select_t\">".html_createInputSelect("nummer_auftraggeber",$db, "id","firma","auftraggeber", "privat=0")."</td>";
$output .= "				</tr>";
$output .= "				<tr>";
$output .= "					<td>".html_createRadioButton("nummer_rb", "rb_above")."</td>";
$output .= "					<td class=\"boxed inactive\" id=\"rb_above_t\">Neu mit Werten von Oben + Firma: ".html_createInputText("ag_firma")."</td>";
$output .= "				</tr>";
$output .= "				<tr>";
$output .= "					<td>".html_createRadioButton("nummer_rb", "rb_new")."</td>";
$output .= "					<td class=\"boxed inactive\" id=\"rb_new_t\">";
$output .= "						<table>	";		
$output .= "							<tr>	";
$output .= "								<td>Firma/Name</td>";
$output .= "								<td>".html_createInputText("ag_firma_full")."</td>";
$output .= "							</tr>	";
$output.="								<tr>";
$output.="									<td>Zusatz:</td>";
$output.="									<td>".html_createInputText("ag_zusatz_full")."</td>";
$output.="								</tr>";
$output.="								<tr>";
$output.="									<td>Strasse:</td>";
$output.="									<td>".html_createInputText("ag_strasse_full")."</td>";
$output.="								</tr>";
$output.="								<tr>";
$output.="									<td>PLZ:</td>";
$output.="									<td>".html_createInputText("ag_plz_full")."</td>";
$output.="								</tr>";
$output.="								<tr>";
$output.="									<td>Ort:</td>";
$output.="									<td>".html_createInputText("ag_ort_full")."</td>";
$output.="								</tr>";
$output .= "						</table>	";			
$output .= "					</td>";
$output .= "				</tr>";
$output .= "			</table>";
$output .= "		</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Anhang</td>";
$output .= "		<td>".html_createInputFile("nummer_file")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>Notizen</td>";
$output .= "		<td>".html_createInputBigText("nummer_notes")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td>".html_createButton("nummer_cancel", "Abbrechen", "cancel('auftragsnummer');")."</td>";
$output .= "		<td>".html_createButton("nummer_save", "Speichern", "saveAuftragsNummer();")."</td>";
$output .= "	</tr>";
$output .= "	<tr>";
$output .= "		<td colspan=\"2\"><i>Felder mit * sind Pflichtfelder!</i></td>";
$output .= "	</tr>";
$output .= "</table>";
}
else {
	$output .= "You have no Permission to get a Auftragsnummer.";
}
echo $output;
?>